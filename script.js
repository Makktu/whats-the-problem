...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function startConversation(initialOption, optionText) {
        ...
        switch(initialOption) {
            case "1":
                initialMessage += "a technical issue (device, software, or connection).";
                break;
            case "2":
    ...
    function displaySolution(response) {
        ...
        const issueMatch = response.match(/\*\*🎯 Here's the likely issue:\*\*\n([\s\S]*?)(?=\n\n\*\*✅|\n\*\*✅)/);
        const stepsMatch = response.match(/\*\*✅ Try this:\*\*\n([\s\S]*?)(?=\n\n\*\*💡|\n\*\*💡|$)/);
        ...
        if (issueMatch && issueMatch.length > 1) {
            ...
            solutionDescription.textContent = "We've identified a potential solution for you.";
        }
        ...
        if (tipMatch && tipMatch.length > 1) {
            ...
            solutionTipText.textContent = "Keep track of what works for you and what doesn't.";
        }
    ...
    copyMarkdownButton.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(markdownLog)
            .then(() => {
